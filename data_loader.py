...
from format_dataset import DIFF_EX_PATH

PARTITIONS = ["train", "valid", "test"]


def get_data_splits(
    comment_type_str=None, ignore_ast=False, include=None, posthoc=False
):
    ...
    if posthoc:
        dataset, high_level_details = load_doctesting_data()
    else:
        dataset, high_level_details = load_processed_data(
            comment_type_str, ignore_ast, include
        )
    train_examples = dataset.get("train", [])
    valid_examples = dataset.get("valid", [])
...
def load_cleaned_test_set(comment_type_str=None):
    ...
    for comment_type in comment_types:
        resources_path = os.path.join(
            DATA_PATH, "resources", comment_type, "clean_test_ids.json"
        )
...
def load_doctesting_data():
    ...
    category_high_level_details_path = os.path.join(
        DATA_PATH, "resources", comment_type, "high_level_details.json"
    )
    ...
    with open(DIFF_EX_PATH, "r") as f:
        for line in f:
            ex = json.loads(line)
            ex["new_ast"] = DiffAST.from_json(ex["new_ast"])
            ex = DiffASTExample(**ex)
            dataset["test"].append(ex)
    return dataset, high_level_details


def load_processed_data(comment_type_str, ignore_ast, include):
    """Processes saved data for the given comment_type_str.
    comment_type_str -- Return, Param, Summary, or None (if None, uses all comment types)
    ignore_ast -- Skip loading ASTs (they take a long time)"""
    if not comment_type_str:
    ...
    for comment_type in comment_types:
        ...
        if not ignore_ast:
            ast_path = os.path.join(
                DATA_PATH, "resources", comment_type, "ast_objs.json"
            )
            with open(ast_path) as f:
                ast_details = json.load(f)

        for partition, examples in loaded.items():
            if include is not None and partition not in include:
                continue
...
def load_raw_data_from_path(path):
    ...
    for partition in PARTITIONS:
        dataset[partition] = []
        dataset[partition].extend(
            read_diff_examples_from_file(
