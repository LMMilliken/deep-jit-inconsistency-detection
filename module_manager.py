...
class ModuleManager(nn.Module):
    ...
    def __init__(
        self,
        attend_code_sequence_states,
        attend_code_graph_states,
        features,
        posthoc,
        task,
    ):
        ...
        print(
            "Attend code sequence states: {}".format(self.attend_code_sequence_states)
        )
    ...
    def initialize(self, train_data):
        ...
        for ex in train_data:
            ...
            if self.attend_code_graph_states:
                code_sequence = [
                    n.value for n in self.get_code_representation(ex, "graph").nodes
                ]
                code_token_counter.update(code_sequence)
        ...
        if self.attend_code_graph_states:
            self.max_ast_length = int(
                np.percentile(np.asarray(sorted(ast_lengths)), LENGTH_CUTOFF_PCT)
            )
        ...
        self.nl_encoder = Encoder(
            NL_EMBEDDING_SIZE, HIDDEN_SIZE, NUM_LAYERS, DROPOUT_RATE
        )
        self.nl_attention_transform_matrix = nn.Parameter(
            torch.randn(
                self.out_dim, self.out_dim, dtype=torch.float, requires_grad=True
            )
        )
        self.self_attention = nn.MultiheadAttention(
            self.out_dim, MULTI_HEADS, DROPOUT_RATE
        )

        if self.encode_code_sequence:
            self.sequence_code_encoder = Encoder(
                CODE_EMBEDDING_SIZE, HIDDEN_SIZE, NUM_LAYERS, DROPOUT_RATE
            )
            self.num_encoders += 1
            self.num_seq_encoders += 1
            ...
            if self.attend_code_sequence_states:
                self.attention_state_size += 2 * HIDDEN_SIZE
                self.sequence_attention_transform_matrix = nn.Parameter(
                    torch.randn(
                        self.out_dim,
                        self.out_dim,
                        dtype=torch.float,
                ...
                self.code_sequence_multihead_attention = nn.MultiheadAttention(
                    self.out_dim, MULTI_HEADS, DROPOUT_RATE
                )

        if self.attend_code_graph_states:
            self.graph_code_encoder = ASTGraphEncoder(
                CODE_EMBEDDING_SIZE, len(DiffEdgeType)
            )
            self.num_encoders += 1
            self.attention_state_size += 2 * HIDDEN_SIZE
            self.graph_attention_transform_matrix = nn.Parameter(
                torch.randn(
                    CODE_EMBEDDING_SIZE,
                    self.out_dim,
                    dtype=torch.float,
            ...
            self.graph_multihead_attention = nn.MultiheadAttention(
                self.out_dim, MULTI_HEADS, DROPOUT_RATE
            )

        if self.features:
            self.code_features_to_embedding = nn.Linear(
                CODE_EMBEDDING_SIZE + get_num_code_features(),
                CODE_EMBEDDING_SIZE,
                bias=False,
            )
            self.nl_features_to_embedding = nn.Linear(
                NL_EMBEDDING_SIZE + get_num_nl_features(), NL_EMBEDDING_SIZE, bias=False
            )
        ...
        if self.generate:
            ...
            self.encoder_final_to_decoder_initial = nn.Parameter(
                torch.randn(
                    self.update_encoder_state_size,
                    self.out_dim,
                    dtype=torch.float,
                    requires_grad=True,
                )
        ...
        if self.classify:
            self.attended_nl_encoder = Encoder(
                self.out_dim, HIDDEN_SIZE, NUM_LAYERS, DROPOUT_RATE
            )
            self.attended_nl_encoder_output_layer = nn.Linear(
                self.attention_state_size, self.out_dim, bias=False
            )
            ...
            for i in range(start_idx, end_idx):
                ...
                if self.generate:
                    ...
                    for c in copy_inputs:
                        ...
                        if (
                            self.embedding_store.is_nl_unk(nl_id)
                            and extra_counter < max_limit
                        ):
                    ...
                    trg_sequence = trg_sequence = (
                        [START] + dataset[i].span_minimal_diff_comment_subtokens + [END]
                    )
                    trg_sequence_ids = self.embedding_store.get_padded_nl_ids(
                        trg_sequence, self.max_nl_length
                    )
                    trg_extended_sequence_ids = (
                        self.embedding_store.get_extended_padded_nl_ids(
                            trg_sequence,
                            self.max_nl_length,
                            ex_inp_ids,
                            ex_inp_str_reps,
                    ...
                    inp_ids.append(
                        self.embedding_store.pad_length(
                            ex_inp_ids, self.max_vocab_extension
                        )
                    )

                    invalid_copy_positions.append(
                        get_invalid_copy_locations(
                            ex_inp_str_reps,
                            self.max_vocab_extension,
                            trg_sequence,
                            self.max_nl_length,
                ...
                if self.features:
                    if self.encode_code_sequence:
                        code_features.append(
                            get_code_features(
                                code_sequence, dataset[i], self.max_code_length
                            )
                        )
                    nl_features.append(
                        get_nl_features(old_nl_sequence, dataset[i], self.max_nl_length)
                    )

            batches.append(
                (
                    UpdateBatchData(
                        torch.tensor(code_token_ids, dtype=torch.int64, device=device),
                        torch.tensor(code_lengths, dtype=torch.int64, device=device),
                        torch.tensor(
                            old_nl_token_ids, dtype=torch.int64, device=device
                        ),
                        torch.tensor(old_nl_lengths, dtype=torch.int64, device=device),
                        torch.tensor(trg_token_ids, dtype=torch.int64, device=device),
                        torch.tensor(
                            trg_extended_token_ids, dtype=torch.int64, device=device
                        ),
                        torch.tensor(trg_lengths, dtype=torch.int64, device=device),
                        torch.tensor(
                            invalid_copy_positions, dtype=torch.uint8, device=device
                        ),
                        inp_str_reps,
                        ...
                        tensorize_graph_method_batch(
                            graph_batch, device, self.max_ast_length
                        ),
                    ),
                    ids,
                )
    ...
    def get_encoder_output(self, batch_data, device):
        ...
        # Encode old NL
        old_nl_embedded_subtokens = self.embedding_store.get_nl_embeddings(
            batch_data.old_nl_ids
        )
        if self.features:
            old_nl_embedded_subtokens = self.nl_features_to_embedding(
                torch.cat([old_nl_embedded_subtokens, batch_data.nl_features], dim=-1)
            )
        old_nl_hidden_states, old_nl_final_state = self.nl_encoder.forward(
            old_nl_embedded_subtokens, batch_data.old_nl_lengths, device
        )
        old_nl_masks = (
            torch.arange(old_nl_hidden_states.shape[1], device=device).view(1, -1)
            >= batch_data.old_nl_lengths.view(-1, 1)
        ).unsqueeze(1)
        attention_states = compute_attention_states(
            old_nl_hidden_states,
            old_nl_masks,
            old_nl_hidden_states,
        ...
        if self.encode_code_sequence:
            code_embedded_subtokens = self.embedding_store.get_code_embeddings(
                batch_data.code_ids
            )
            if self.features:
                code_embedded_subtokens = self.code_features_to_embedding(
                    torch.cat(
                        [code_embedded_subtokens, batch_data.code_features], dim=-1
                    )
                )
            code_hidden_states, code_final_state = self.sequence_code_encoder.forward(
                code_embedded_subtokens, batch_data.code_lengths, device
            )
            code_masks = (
                torch.arange(code_hidden_states.shape[1], device=device).view(1, -1)
                >= batch_data.code_lengths.view(-1, 1)
            ).unsqueeze(1)
            ...
            if self.attend_code_sequence_states:
                attention_states = torch.cat(
                    [
                        attention_states,
                        compute_attention_states(
                            code_hidden_states,
                            code_masks,
                            old_nl_hidden_states,
                            transformation_matrix=self.sequence_attention_transform_matrix,
                            multihead_attention=self.code_sequence_multihead_attention,
                        ),
                    ],
                    dim=-1,
                )
        ...
        if self.attend_code_graph_states:
            embedded_nodes = self.embedding_store.get_node_embeddings(
                batch_data.graph_batch.value_lookup_ids,
                batch_data.graph_batch.src_type_ids,
            )

            if self.features:
                embedded_nodes = self.code_features_to_embedding(
                    torch.cat(
                        [embedded_nodes, batch_data.graph_batch.node_features], dim=-1
                    )
                )

            graph_states = self.graph_code_encoder.forward(
                embedded_nodes, batch_data.graph_batch, device
            )
            graph_lengths = batch_data.graph_batch.num_nodes_per_graph
            graph_masks = (
                torch.arange(graph_states.shape[1], device=device).view(1, -1)
                >= graph_lengths.view(-1, 1)
            ).unsqueeze(1)

            transformed_graph_states = torch.einsum(
                "ijk,km->ijm", graph_states, self.graph_attention_transform_matrix
            )
            graph_attention_states = compute_attention_states(
                transformed_graph_states,
                graph_masks,
                old_nl_hidden_states,
                multihead_attention=self.graph_multihead_attention,
            )
            attention_states = torch.cat(
                [attention_states, graph_attention_states], dim=-1
            )

        if self.classify:
            nl_attended_states = torch.tanh(
                self.attended_nl_encoder_output_layer(attention_states)
            )
            _, attended_old_nl_final_state = self.attended_nl_encoder.forward(
                nl_attended_states, batch_data.old_nl_lengths, device
            )
        else:
        ...
        if self.generate:
            ...
            encoder_hidden_states, input_lengths = merge_encoder_outputs(
                ...
                input_lengths,
                old_nl_hidden_states,
                batch_data.old_nl_lengths,
                device,
            )
            mask = (
                torch.arange(encoder_hidden_states.shape[1], device=device).view(1, -1)
                >= input_lengths.view(-1, 1)
            ).unsqueeze(1)
        else:
