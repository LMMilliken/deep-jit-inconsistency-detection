...
def train(model, train_examples, valid_examples):
    """Trains a model."""
    print(
        "Training with {} examples (validation {})".format(
            len(train_examples), len(valid_examples)
        )
    )
    sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_mode", action="store_true", help="whether to run evaluation"
    )
    parser.add_argument(
        "--rerank",
        action="store_true",
    ...
    parser.add_argument(
        "--model_name", help="name of model (used to save model output)"
    )
    args = parser.parse_args()

    train_examples, valid_examples, test_examples, high_level_details = get_data_splits(
        posthoc=args.posthoc and args.test_mode
    )
    ...
    if args.test_mode:
        ...
        print(
            "Terminating evaluation: {}".format(
                datetime.now().strftime("%m/%d/%Y %H:%M:%S")
            )
        )
    else:
        print(
            "Starting training: {}".format(datetime.now().strftime("%m/%d/%Y %H:%M:%S"))
        )

        manager = ModuleManager(
            args.attend_code_sequence_states,
            args.attend_code_graph_states,
            args.features,
            args.posthoc,
            args.task,
        )
